...
function hotkeys:init(args)
  ...
  local menu_keys_move = {
    {
      { env.mod },
      "k",
      redflat.menu.action.down,
      { description = "Select next item", group = "Navigation" },
    },
    {
      { env.mod },
      "i",
      redflat.menu.action.up,
      { description = "Select previous item", group = "Navigation" },
    },
    {
      { env.mod },
      "j",
      redflat.menu.action.back,
      { description = "Go back", group = "Navigation" },
    },
    {
      { env.mod },
      "l",
      redflat.menu.action.enter,
  ...
  self.raw.root = {
    ...
    },
    {
      ...
      { description = "Focus next screen", group = "Tag navigation" },
    },
    {
      { env.mod, "Shift" },
      "Left",
      ...
      { description = "Focus previous screen", group = "Tag navigation" },
    },
    {
      { env.mod },
      "y",
  ...
  for i = 1, 9 do
    self.keys.root = awful.util.table.join(
      self.keys.root,
      tag_numkey(i, { env.mod }, function(t)
        t:view_only()
      end),
      tag_numkey(i, { env.mod, "Control" }, function(t)
        awful.tag.viewtoggle(t)
      end),
      client_numkey(i, { env.mod, "Shift" }, function(t)
        client.focus:move_to_tag(t)
      end),
      client_numkey(i, { env.mod, "Control", "Shift" }, function(t)
